	...
	glm::vec3 const& V)			// view vector (already normalized)
{
	...
	glm::vec3 kblue = glm::vec3(0.6f, 0.6f, 0.f);
	glm::vec3 kyellow = glm::vec3(0.f, 0.f, 0.6f);
	float alpha = 0.3f;
	float beta = 0.3f;

	glm::vec3 kcool = kblue + alpha * diffuse;
	glm::vec3 kwarm = kyellow + beta * diffuse;

	float gooch = (1.0f + dot(L,N)) /2.0f;

	glm::vec3 kfinal(0.f);
	kfinal = gooch * kcool + (1.f - gooch) * kwarm;

	return kfinal + specular;
}
